...
- `backup.py`：生成、校验、轮换和恢复 SQLite 一致性快照。
...
原始 SQLite 数据库不上传 GitHub。GitHub 周总结用于长期保留可读总结，不作为逐字段数据库备份。

备份目录：

```text
data/backups/sqlite/
├── tasks-2026-07-25-080000.db
├── tasks-2026-07-24-081000.db
└── tasks-2026-07-23-075500.db
```

生成规则：

1. 每个上海本地自然日，在 Sakura 或任一后台 worker 第一次运行时生成一份。
2. schema migration 和批量数据清理前额外生成一份。
3. 使用 SQLite 在线备份接口生成一致性快照，不直接复制正在运行的 `tasks.db`、WAL 和 SHM 文件。
4. 先写入临时文件，执行 `PRAGMA integrity_check`；只有结果为 `ok` 才原子改名为正式备份。
5. 多进程通过维护锁保证同一个每日备份最多生成一次。
...
- 超过任一限制时，从最旧快照开始删除。
- 新快照验证成功前不得删除旧快照。
- 无论空间限制如何，至少保留最新一份有效快照；若单份快照已经超过 200 MB，则保留并提示用户。

数据清理完成后，先生成并验证一份清理后的新快照。清理前快照作为回滚保护最多额外保留 24 小时，之后删除；因此刚被清理的原始记录可能在本地恢复快照中多保留最多 24 小时，但不会被 Sakura 查询，也不会上传网络。

恢复流程必须由用户明确确认：

1. 停止 Sakura 的任务写入及两个后台 worker。
2. 把损坏数据库移动到带时间戳的隔离文件，不直接覆盖。
3. 从新到旧检查备份，选择最近一份 `integrity_check = ok` 的快照。
4. 恢复到临时数据库并再次执行完整性检查。
5. 原子替换 `tasks.db`。
6. 重新启动服务并报告备份时间以及可能丢失的时间范围。

本地滚动备份能够处理应用错误、数据库损坏和错误清理，但不能处理整块硬盘丢失。第一版不实现异地原始数据库备份；后续如有需要，应设计加密云盘或外接硬盘方案，而不是把 SQLite 文件提交到 Git。
...
- 事务失败：完整回滚。
- 数据库损坏：停止写入和清理，列出最近的有效本地备份，等待用户确认后恢复。
- 没有有效备份：保留损坏文件并停止自动操作，不创建空数据库覆盖原文件。
...
- JSON 迁移重复运行不重复导入。
- 在线备份包含 WAL 中已提交的数据并通过完整性检查。
- 备份轮换同时遵守 7 份和 200 MB 上限，且不会删除最后一份有效快照。
- 损坏备份不会被选作恢复来源。
- 未经用户确认不会替换当前数据库。
...
17. 每日和清理前备份能够生成一致快照，并按 7 份、200 MB 上限安全轮换。
18. 数据库损坏时可以在用户确认后从最近的有效快照恢复。

## 21. 实施边界

本文档批准后，下一步应编写单独的实施计划，并按测试驱动方式分阶段实现：

1. SQLite schema、模型、仓储和滚动备份。
